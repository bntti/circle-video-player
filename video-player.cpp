...
int main(int argc, char **argv) {
    ...
    if (help || filename == "") {
        ...
        std::cout << "\t'-r' | Use larger radius for circles allowing for them to fully fill an area.\n";
        std::cout << "\t'-s <target_pixel_size>' | Size of one 'pixel'. Use smaller number for more circles (integer).\n";
        exit(0);
    }
    ...
    if (!cap.isOpened()) {
        std::cout << "Error opening video stream or file '" << input_file << "'"
                  << std::endl;
        return 1;
    }
    ...
    while (1) {
            ...
            for (int y = 0; y < im_height; ++y) {
                ...
                if (newradius > 0) {
                    ...
                    circle.setPosition(sf::Vector2f(
                        x * pixel_size + pixel_size / 2.0 - newradius,
        ...
        if (time_now_ms < current_frame_time_ms)
            std::this_thread::sleep_for(
                std::chrono::milliseconds(current_frame_time_ms - time_now_ms));
    }
}
